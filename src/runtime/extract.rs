...
fn describe_issue_in_chinese(summary: &str, group_ordinals: &[usize]) -> String {
    ...
    if let Some(rest) = summary.strip_prefix("unsupported payload: action button payload too short (")
    ...
    {
        return format!(
            "{target} 暂时无法导出，因为按钮动作类型 ({action_kind}) 目前还不支持。"
        );
    }
    ...
    if let Some(rest) =
        summary.strip_prefix("unsupported payload: non-positive image dimensions (")
...
fn describe_group_in_chinese(file: &GspFile, groups: &[ObjectGroup], group: &ObjectGroup) -> String {
    ...
    let mut detail = match group.header.kind() {
        ...
        GroupKind::Circle => {
            if refs.len() == 2 {
                format!("圆，圆心是 {}，并且经过 {}", format_ref(refs[0]), format_ref(refs[1]))
            } else {
        ...
        GroupKind::SegmentMarker => refs
            .first()
            .map(|host| format!("用于标记 {} 的线段记号", format_ref_with_kind(groups, *host)))
    ...
    if let Some(url) = self::decode::decode_link_button_url(file, group) {
        annotations.push(format!("链接“{}”", truncate_text(url.trim(), 64)));
    }
...
fn describe_point_group_in_chinese(file: &GspFile, refs: &[usize], group: &ObjectGroup) -> String {
    let has_explicit_point = group.records.iter().any(|record| record.record_type == 0x0899);
    let has_image_payload = [0x090c, 0x08a8, 0x1f44]
        .into_iter()
        .all(|record_type| group.records.iter().any(|record| record.record_type == record_type));
    ...
    let point = group
        .records
        .iter()
        .find(|record| record.record_type == 0x0899)
...
fn describe_pair_relation(refs: &[usize], noun: &str, verb: &str) -> String {
    if refs.len() == 2 {
        format!("{noun}，{verb} {} 和 {}", format_ref(refs[0]), format_ref(refs[1]))
    } else {
...
fn describe_intersection_point(refs: &[usize], variant: Option<&str>) -> String {
    if refs.len() >= 2 {
        ...
        format!("{prefix}交点，来自 {} 和 {}", format_ref(refs[0]), format_ref(refs[1]))
    } else {
        "交点".to_string()
    }
}
...
fn describe_circle_intersection_point(refs: &[usize], variant: Option<&str>) -> String {
    if refs.len() >= 2 {
        let prefix = variant.unwrap_or("");
        format!("{prefix}圆交点，来自 {} 和 {}", format_ref(refs[0]), format_ref(refs[1]))
    } else {
        "圆交点".to_string()
    }
}
...
fn describe_rotation_group_in_chinese(
    ...
    describe_generic_group(group, &find_indexed_path(file, group).map(|path| path.refs).unwrap_or_default())
}
...
fn describe_function_plot_group_in_chinese(groups: &[ObjectGroup], refs: &[usize]) -> String {
    ...
    if refs.len() == 1 {
        return format!("函数图像，定义来自 {}", format_ref_with_kind(groups, refs[0]));
    }
    "函数图像".to_string()
}
    ...
    match group.header.kind() {
        GroupKind::Unknown(raw) => {
            if refs.is_empty() {
                ...
                format!("未知对象，类型是 {raw}，按载荷顺序引用 {}", format_ref_list(refs))
            }
        }
        kind => {
...
fn format_ref_with_kind(groups: &[ObjectGroup], ordinal: usize) -> String {
    groups
        ...
        .map(|group| format!("{} #{}", group_kind_noun_in_chinese(group.header.kind()), ordinal))
...
fn format_record_summary(file: &GspFile, record: &Record) -> Option<String> {
    ...
    match record.record_type {
        0x0899 => decode_point_record(payload)
            .map(|point| format!("点=({:.3}, {:.3})", point.x, point.y)),
