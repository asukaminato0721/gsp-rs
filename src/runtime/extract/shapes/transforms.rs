...
pub(crate) fn collect_rotated_line_shapes(
    ...
    groups
        .iter()
        .filter(|group| (group.header.kind()) == 29)
        .filter_map(|group| {
...
pub(crate) fn collect_scaled_line_shapes(
    ...
    groups
        .iter()
        .filter(|group| (group.header.kind()) == 30)
        .filter_map(|group| {
...
pub(crate) fn collect_reflected_line_shapes(
    ...
    groups
        .iter()
        .filter(|group| matches!(group.header.kind(), 16 | 34))
        .filter_map(|group| {
            let path = find_indexed_path(file, group)?;
            let source_group = groups.get(path.refs.first()?.checked_sub(1)?)?;
            if (source_group.header.kind()) != 2 {
...
pub(crate) fn collect_translated_polygon_shapes(
    ...
    groups
        .iter()
        .filter(|group| (group.header.kind()) == 16)
        .filter_map(|group| {
            let path = find_indexed_path(file, group)?;
            let source_group = groups.get(path.refs.first()?.checked_sub(1)?)?;
...
pub(crate) fn collect_reflected_circle_shapes(
    ...
    groups
        .iter()
        .filter(|group| (group.header.kind()) == 34)
        .filter_map(|group| {
            let path = find_indexed_path(file, group)?;
            let source_group = groups.get(path.refs.first()?.checked_sub(1)?)?;
            if (source_group.header.kind()) != 3 {
