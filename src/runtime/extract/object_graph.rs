...
impl Builder {
        ...
        if let Some(binding) = &point.binding {
            match binding {
                ...
                ScenePointBinding::Parameter { name } => {
                    ...
                    } else if let Some(parameter_id) = self.named_scalars.get(name).cloned() {
                        if !self.point_at_parameter(id.clone(), index, parameter_id) {
                            self.pending_source(id, "point-parameter-binding", source_value);
                        }
                    } else {
                        self.pending_source(id, "point-parameter-source", source_value);
                    }
                }
            ...
            ScenePointConstraint::Free => {
                if let Some(debug) = &point.debug
                    && let variant = match debug.group_kind {
                        ...
                        | GroupKind::CoordinateTraceIntersectionPoint => Some(0),
    ...
    fn point_trace_or_repeat(
        ...
        self.derived(
            id,
            ObjectOp::Curve {
                curve: CurveOp::RepeatPoint { sample_count },
            },
            [target_id],
        );
        true
        ...
        let (
            ...
            ScenePointBinding::Coordinate { name, expr } => {
                ...
                self.derived(
                    id,
                    ObjectOp::Curve {
                        curve: CurveOp::CartesianParameterTrace {
                            expression: ObjectExpression::from_function_expr(&expr),
                            parameter_names,
