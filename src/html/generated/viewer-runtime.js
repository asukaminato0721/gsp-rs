...
(function() {
  ...
  */
  ...
  scene.registerPointConstraintResolver(
    "circle",
    /** @type {any} */
    ((_env, constraint, resolveFn) => {
      const center = resolveFn(constraint.centerIndex);
      const radiusPoint = resolveFn(constraint.radiusIndex);
      if (!center || !radiusPoint) return null;
  ...
  scene.registerLineBindingResolver(
    "arc-boundary",
    /** @type {any} */
    ((env, line) => sampleArcBoundaryPoints(env, line.binding))
  );
...
(function() {
  ...
  */
  ...
  scene.registerLineBindingResolver(
    "coordinate-trace",
    /** @type {any} */
    ((env, line) => sampleCoordinateTracePoints(env, line.binding))
  );
...
(function() {
  ...
  */
  ...
  scene.registerPointConstraintResolver(
    "point-circular-tangent",
    /** @type {any} */
    ((env, constraint, resolveFn, reference) => {
  ...
  scene.registerPointConstraintResolver(
    ...
    /** @type {any} */
    ((_env, constraint, resolveFn, reference) => {
  ...
  function deriveExpressionLabelParameters(scene, seedParameters) {
    ...
    for (let pass = 0; pass < maxPasses; pass += 1) {
      ...
      scene.labels.forEach((label) => {
        ...
        if ((binding.kind === "segment-parameter" || binding.kind === "segment-projection-parameter" || binding.kind === "polyline-parameter" || binding.kind === "polygon-boundary-parameter" || binding.kind === "circle-parameter") && typeof binding.pointName === "string") {
    ...
    Object.entries(
      ...
      if (key === "expr" && child && typeof child === "object") {
        addExprParameterDeps(
          deps,
          /** @type {FunctionExprJson | FunctionAstJson} */
          child,
          knownParameters,
  ...
  */
  function parameterNameFromPoint(scene, pointIndex) {
    for (const label of scene.labels || []) {
      ...
      if (binding.kind === "segment-parameter" || binding.kind === "polyline-parameter" || binding.kind === "polygon-boundary-parameter" || binding.kind === "circle-parameter") {
        return binding.pointName;
      }
    }
    return null;
  ...
  */
  ...
  function buildParameterControls(env) {
    ...
    const controls = env.currentDynamics().parameters.map((parameter, index) => ({
      ...
      return env.labelTag(`${parameter.name} =`, env.inputTag(
        /** @type {Parameters<ViewerEnv["inputTag"]>[0]} */
        inputAttrs
...
(() => {
  ...
  */
  function isDocumentSceneData(data) {
    return !!data && typeof data === "object" && data.kind === "gsp-document" && Array.isArray(
      /** @type {{ pages?: unknown }} */
      data.pages
    );
  }
  ...
  */
  function cloneWithLiveParameterValues(value, parameters) {
    ...
    const cloned = Object.fromEntries(Object.entries(
      /** @type {Record<string, unknown>} */
      value
    ).map(([key, child]) => [key, cloneWithLiveParameterValues(child, parameters)]));
