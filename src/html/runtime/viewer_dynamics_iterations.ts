(function() {
  ...
  function rebuildIterationPoints(env: ViewerEnv, _scene: ViewerSceneData, _parameters: Map<string, number>) {
    // Point iterations are evaluated by the Rust object-graph interpreter. This pass only
    // validates the scene contract; it must not duplicate the operation in TypeScript.
    (env.sourceScene.pointIterations || []).forEach((family) => {
      if (family.kind !== "interpreted") {
        throw new Error(`unsupported point iteration kind: ${(family as any).kind}`);
      }
    });
  }
